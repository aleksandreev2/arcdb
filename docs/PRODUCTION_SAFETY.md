...
The repository also provides `scripts/verify_read_cutover_readiness.py` as a read-only gate. It requires explicit live paths, requires the three core legacy documents, verifies schema/quick/integrity/foreign-key health plus complete schema-v3 parity in one SQLite snapshot, and proves the recursively discovered legacy sources did not change during the run. Known derived/operational SQLite files below that root must be named explicitly with repeatable `--exclude-derived-db`; unknown files are never excluded by suffix or guess. Its optional report omits paths, identities and payloads, refuses overwrite and deliberately sets canary/primary authorization to false. Passing this tool is evidence for operator review, not permission to cut over.
