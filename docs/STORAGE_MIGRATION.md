...
- authenticated administrators can call `GET /api/admin/state-read-probe` during a
  bounded shadow-observation window to exercise all six read adapters; the response
  contains only fixed domain names/status, never counts, payloads or identities;
